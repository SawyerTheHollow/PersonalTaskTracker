...
/// Signature used by [TaskaDismissible] to indicate that it has been dismissed in
/// the given `direction`.
///
/// Used by [TaskaDismissible.onDismissed].
typedef DismissDirectionCallback = void Function(DismissDirection direction);

/// Signature used by [TaskaDismissible] to give the application an opportunity to
/// confirm or veto a dismiss gesture.
///
/// Used by [TaskaDismissible.confirmDismiss].
typedef ConfirmDismissCallback = Future<bool?> Function(DismissDirection direction);

/// Signature used by [TaskaDismissible] to indicate that the dismissible has been dragged.
///
/// Used by [TaskaDismissible.onUpdate].
typedef DismissUpdateCallback = void Function(DismissUpdateDetails details);

/// The direction in which a [TaskaDismissible] can be dismissed.
enum DismissDirection {
  /// The [TaskaDismissible] can be dismissed by dragging either up or down.
  vertical,

  /// The [TaskaDismissible] can be dismissed by dragging either left or right.
  horizontal,

  /// The [TaskaDismissible] can be dismissed by dragging in the reverse of the
  /// reading direction (e.g., from right to left in left-to-right languages).
  endToStart,

  /// The [TaskaDismissible] can be dismissed by dragging in the reading direction
  /// (e.g., from left to right in left-to-right languages).
  startToEnd,

  /// The [TaskaDismissible] can be dismissed by dragging up only.
  up,

  /// The [TaskaDismissible] can be dismissed by dragging down only.
  down,

  /// The [TaskaDismissible] cannot be dismissed by dragging.
  none,
}
...
/// non-null, the TaskaDismissible widget animates its height (or width, whichever is
...
///
/// {@tool dartpad}
/// This sample shows how you can use the [TaskaDismissible] widget to
...
/// is specified it is stacked behind the TaskaDismissible's child and is exposed when
/// the child moves.
///
/// The widget calls the [onDismissed] callback either after its size has
/// collapsed to zero (if [resizeDuration] is non-null) or immediately after
/// the slide animation (if [resizeDuration] is null). If the TaskaDismissible is a
/// list item, it must have a key that distinguishes it from the other items and
/// its [onDismissed] callback must remove the item from the list.
class TaskaDismissible extends StatefulWidget {
  /// Creates a widget that can be dismissed.
  ///
  /// The [key] argument is required because [TaskaDismissible]s are commonly used in
  ...
  @override
  State<TaskaDismissible> createState() => _DismissibleState();
}
...
/// See also:
///
///   * [TaskaDismissible.onUpdate], which receives this information.
class DismissUpdateDetails {
  ...
  /// that the [TaskaDismissible] is dragged across the threshold.
...
class _DismissibleState extends State<TaskaDismissible>
      ...
      assert(() {
        if (_resizeAnimation!.status != AnimationStatus.forward) {
          ...
          throw FlutterError.fromParts(<DiagnosticsNode>[
            ErrorSummary('A dismissed TaskaDismissible widget is still part of the tree.'),
            ErrorHint(
              'Make sure to implement the onDismissed handler and to immediately remove the TaskaDismissible '
